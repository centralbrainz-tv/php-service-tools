...
for ($index = 916583; $index <= 1000001; $index++) {
    if (!in_array($index, $broken)) {
        ...
        if ($doc) {
            $titleYear = $doc->getElementById("titleYear")
                ? $doc->getElementById("titleYear")->plaintext
                : '';
            if ($doc->getElementById("titleYear")) {
            ...
            $poster = "";
            if (
                count($doc->find('.poster')) > 0 &&
                count($doc->find('.poster')[0]->find('a')) > 0
            ) {
                $poster = $doc
                    ->find('.poster')[0]
                    ->find('a')[0]
            ...
            $docFullCredit = $htmlFullCredit
                ? str_get_html($htmlFullCredit)
                : null;
            $h4FullCredit = $docFullCredit
                ? $docFullCredit->find('#fullcredits_content')[0]->find('h4')
                : '';
            $tableFullCredit = $docFullCredit
                ? $docFullCredit->find('#fullcredits_content')[0]->find('table')
                : [];
            $arrayFullCredit = [];
            ...
            $docPlotSummary = $htmlPlotSummary
                ? str_get_html($htmlPlotSummary)
                : null;
            $plotSummaryName = $docPlotSummary
                ? $docPlotSummary->find('#summaries')[0]->innertext
                : "Summaries";

            $plotSummaryUl =
                $docPlotSummary &&
                count($docPlotSummary->find('#plot-summaries-content')) > 0
                    ? $docPlotSummary
                        ->find('#plot-summaries-content')[0]
                        ->find('li')
                    : [];
            ...
            $synopsisName = $docPlotSummary
                ? $docPlotSummary->find('#synopsis')[0]->innertext
                : "Summaries";
            $synopsisUl = $docPlotSummary
                ? $docPlotSummary->find('#plot-synopsis-content')[0]->find('li')
                : [];
            ...
            if (
                $docKeywords &&
                count($docKeywords->find('.dataTable.evenWidthTable2Col')) > 0
            ) {
                $tableKeywords = $docKeywords->find(
                    '.dataTable.evenWidthTable2Col'
                )[0];
                for (
                    $k = 0;
                    $k <
                    count(
                        $docKeywords
                            ->find('.dataTable.evenWidthTable2Col')[0]
                            ->find('.did-you-know-actions')
                    );
                    $k++
                ) {
                    $docKeywords
                        ->find('.dataTable.evenWidthTable2Col')[0]
                        ->find('.did-you-know-actions')[$k]->outertext = '';
                }
            ...
            $docParentalGuide = $htmlParentalGuide
                ? str_get_html($htmlParentalGuide)
                : null;

            $arrayParentalGuide = [];
            $sections = $docParentalGuide
                ? $docParentalGuide
                    ->find('section.article.listo.content-advisories-index')[0]
                    ->find('section')
                : [];
            ...
            for ($i = 0; $i < count($sections); $i++) {
                ...
                if ($i === 0) {
                    $name = $section->find('header')[0]->find('h4')[0]
                        ->innertext;
                    $text = '';
                    if (count($section->find('table')) > 0) {
                        $text = compress_htmlcode(
                            $section->find('table')[0]->outertex
                        );
                    }

                    $arrayParentalGuide[$i] = [
                        'id' => $i,
                        'name' => $name,
                        'text' => [$text],
                    ];
                    ...
                    for ($j = 0; $j < count($texts); $j++) {
                        for (
                            ...
                            count(
                                $texts[$j]->find(
                                    '.ipl-hideable-container.ipl-hideable-container--hidden.ipl-zebra-list__action-row'
                                )
                            );
                            $k++
                        ) {
            ...
            $docReleaseInfo = $htmlReleaseInfo
                ? str_get_html($htmlReleaseInfo)
                : null;
            ...
            $h4Arr =
                $docReleaseInfo &&
                count($docReleaseInfo->find('#releaseinfo_content')) > 0
                    ? $docReleaseInfo
                        ->find('#releaseinfo_content')[0]
                        ->find('h4')
                    : [];
            $tableArr =
                $docReleaseInfo &&
            ...
            $docLocations = $htmlLocations
                ? str_get_html($htmlLocations)
                : null;
            ...
            if (
                $docLocations &&
                count(
                    $docLocations->find('h4.ipl-header__content.ipl-list-title')
                ) > 0
            ) {
                $nameLocations = $docLocations->find(
                    'h4.ipl-header__content.ipl-list-title'
                )[0]->innertext;
            ...
            $dates = ['id' => 0, 'name' => 'Filming Dates', 'text' => ''];
            if (
                $docLocations &&
                count($docLocations->find('#filming_dates')) > 0
            ) {
                $nameDates = $docLocations
                    ->find('#filming_dates')[0]
                    ->find('h4')[0]->innertext;
                ...
                $dates = ['id' => 0, 'name' => $nameDates, 'text' => $texts];
            }

            $urlTechnical = $url . 'technical/';
            $htmlTechnical = file_get_html($urlTechnical);
            $docTechnical = $htmlTechnical
                ? str_get_html($htmlTechnical)
                : null;
            ...
            if (
                $docTechnical &&
                count($docTechnical->find('table.dataTable.labelValueTable')) >
                    0
            ) {
                $text = compress_htmlcode(
                    $docTechnical->find('table.dataTable.labelValueTable')[0]
                        ->outertext
                );
            ...
            $faqHeads = $docFAQ
                ? $docFAQ->find('.ipl-header__content.ipl-list-title')
                : [];
            ...
            for ($j = 0; $j < count($faqHeads); $j++) {
                ...
                for (
                    $i = 0;
                    $i < count($faqUls[$j]->find('div.faq-question-text'));
                    $i++
                ) {
                    $text = $faqUls[$j]->find('div.faq-question-text')[$i]
                        ->innertext;
                    $faqTexts[$i] = $text;
                }
                $arrayFAQ[$i] = [
            ...
            $awardsHeads = $docAwards
                ? $docAwards->find('.article.listo')[0]->find('h3')
            ...
            for ($j = 0; $j < count($awardsTables); $j++) {
                $arrayAwards[$j] = [
                    'id' => $j,
                    'name' => trim_whitespace($awardsHeads[$j]->plaintext),
                    'text' => compress_htmlcode($awardsTables[$j]->outertext),
                ];
            }

            $urlSoundtrack = $url . 'soundtrack/';
            $htmlSoundtrack = file_get_html($urlSoundtrack);
            $docSoundtrack = $htmlSoundtrack
                ? str_get_html($htmlSoundtrack)
                : null;
            ...
            if (
                $docSoundtrack &&
                count($docSoundtrack->find('#soundtracks_content')) > 0 &&
                count(
                    $docSoundtrack->find('#soundtracks_content')[0]->find('h4')
                ) > 0 &&
                count($docSoundtrack->find('#no_content')) <= 0
            ) {
                $nameSoundtrack = $docSoundtrack
                    ->find('#soundtracks_content')[0]
                    ->find('h4')[0]->innertext;
            ...
            if ($docTrivia && count($docTrivia->find('#no_content')) <= 0) {
                ...
                $trivias = ['id' => 0, 'name' => 'Trivia', 'text' => $texts];
            }
            ...
            if ($docQuotes && count($docQuotes->find('#no_content')) <= 0) {
                ...
                $quotes = ['id' => 0, 'name' => 'Quotes', 'text' => $texts];
            }
            ...
            if ($docGoofs && count($docGoofs->find('#no_content')) <= 0) {
                ...
                $goofs = ['id' => 0, 'name' => 'Goofs', 'text' => $texts];
            }
            ...
            $CC = ['id' => 0, 'name' => 'Crazy Credits', 'text' => ''];
            ...
            if ($docCC && count($docCC->find('#no_content')) <= 0) {
                ...
                $CC = ['id' => 0, 'name' => 'Crazy Credits', 'text' => $texts];
            }
            ...
            $AV = ['id' => 0, 'name' => 'Alternate Versions', 'text' => ''];
            ...
            if ($docAV && count($docAV->find('#no_content')) <= 0) {
                ...
                $AV = ['id' => 0, 'name' => 'Crazy Credits', 'text' => $texts];
            }
            ...
            if ($docMC && count($docMC->find('#no_content')) <= 0) {
                ...
                $MC = ['id' => 0, 'name' => 'Crazy Credits', 'text' => $texts];
            }
